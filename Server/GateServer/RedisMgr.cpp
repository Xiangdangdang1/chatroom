...
RedisMgr::RedisMgr() 
{
	...
	_redisPool.reset(new RedisConPool(5, host.c_str(), atoi(port.c_str()), pwd.c_str()));   //获取链接 
}
...
//调用hiredis库的函数执行GET命令获取key对应的value
bool RedisMgr::Get(const std::string& key, std::string& value)
{
	...
	//判断返回值类型是否为字符串,如果不是字符串类型，说明执行命令失败
	...
	_redisPool->returnConnection(connect); //归还连接
    return true;
}


//调用hiredis库的函数执行SET命令设置key对应的value
